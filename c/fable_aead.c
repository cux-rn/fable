/* Fable-AEAD / Fable-XOF, C port of fable.py (v0.1). Research draft, NOT for production. */
#include <string.h>
...
static void aead_init(uint32_t s[16], uint32_t k[8], const uint8_t key[32], const uint8_t nonce[24]) {
    for (int j = 0; j < 8; j++) k[j] = ld32(key + 4 * j);
    s[0] = IV0_AEAD; s[1] = IV1_AEAD;
...
static void absorb_ad(uint32_t s[16], const uint8_t *ad, size_t adlen) {
    if (adlen) {
        size_t full = adlen / RATE_BYTES;
        for (size_t i = 0; i < full; i++) {
            xor_rate(s, ad + i * RATE_BYTES);
            fable_permute(s, FABLE_ROUNDS_LIGHT);
        }
