...
/* Fixed-rotation fast path: rotation amounts are compile-time constants here, so the
 * compiler emits immediate-count rotates instead of variable shifts. */
void fable_permute(uint32_t s[16], int rounds) {
    uint32_t s0 = s[0], s1 = s[1], s2 = s[2], s3 = s[3];
    uint32_t s4 = s[4], s5 = s[5], s6 = s[6], s7 = s[7];
    uint32_t s8 = s[8], s9 = s[9], s10 = s[10], s11 = s[11];
    uint32_t s12 = s[12], s13 = s[13], s14 = s[14], s15 = s[15];
    const int r0 = FABLE_R0, r1 = FABLE_R1, r2 = FABLE_R2, r3 = FABLE_R3;
    for (int i = FABLE_ROUNDS_HEAVY - rounds; i < FABLE_ROUNDS_HEAVY; i++) {
